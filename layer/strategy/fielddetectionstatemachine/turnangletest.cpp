...
void TurnAngleTest::nextState_targetReached_detectField()
{
	...
	RefereeMock referee;
	LoggerMock logger;
	TurnAngle turnAngleState(robot, field, referee, logger, Angle::getThreeQuarterRotation(), 0);
...
void TurnAngleTest::update_quarterRotation_robotGetsCorrectTarget()
{
	...
	RefereeMock referee;
	LoggerMock logger;
	TurnAngle turnAngleState(robot, field, referee, logger, Angle::getQuarterRotation(), 0);
...
void TurnAngleTest::update_halfRotation_robotGetsCorrectTarget()
{
	...
	RefereeMock referee;
	LoggerMock logger;
	TurnAngle turnAngleState(robot, field, referee, logger, Angle::getHalfRotation(), 0);
