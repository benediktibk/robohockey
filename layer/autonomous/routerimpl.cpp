		...
		const RobotPosition &start, const RobotPosition &end, const Field &field) const
{
	...
	Path startPart(start.getPosition(), start.getPosition(), sqrt(2)*m_robotWidth);
	if (startPart.intersectsWith(hardObstacles))
		return Route(m_robotWidth);

	Path endPart(end.getPosition(), end.getPosition(), sqrt(2)*m_robotWidth);
...
vector<Point> RouterImpl::getPointsBesideObstacle(const Path &path, const Circle &obstacle) const
{
	...
	double offsetDistanceLongPoint = 0.5*(1.01 + sqrt(2))*m_robotWidth;
	...
	if(intersectionPoints.getIntersectTypeFrom() != PathIntersectPoints::IntersectTypeNoIntersect)
	{
		...
		double offsetDistanceShortPoint = sqrt(2)*0.51*m_robotWidth + distanceIntersectPointsCenterToCircle;
		offsetDistanceLongPoint = offsetDistanceLongPoint + distanceIntersectPointsCenterToCircle;
		...
		const vector<Circle> &obstacles, unsigned int searchDepth, bool canGoLeft, bool canGoRight) const
{
	...
	Path endPart(end, end, sqrt(2)*m_robotWidth);
...
{
	Path path(end, end, sqrt(2)*m_robotWidth);
