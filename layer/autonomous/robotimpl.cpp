...
RobotImpl::RobotImpl(DataAnalysis::DataAnalyser *dataAnalyser, Router *router, const Watch &watch, Common::Logger &logger, bool enableSonar) :
	...
	m_timeout(30),
	m_maximumAngleForSmoothTurn(Angle::getHalfRotation()/4),
...
bool RobotImpl::isInDrivingTurningPart() const
{
	return m_state == RobotStateDrivingTurningPart;
}

bool RobotImpl::isInDrivingStraightPart() const
{
	return m_state == RobotStateDrivingStraightPart;
}
